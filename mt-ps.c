...
int main(int argc, char **argv)
{
	...
	int total_ports, extra_port, ports_per_thread, num_threads, s;
	...
	extra_port = total_ports % NUM_THREADS;
	ports_per_thread = total_ports / NUM_THREADS;

	num_threads = (ports_per_thread == 0) ? 1 : NUM_THREADS;
	for (int tnum = 0; tnum < num_threads; tnum++) {
		...
		s = pthread_create(&tinfo[tnum].thread_id, NULL, &scanner,
				   &tinfo[tnum]);
		if (s != 0)
